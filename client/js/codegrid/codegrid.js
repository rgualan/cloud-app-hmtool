...
// Configuration. Should match generator settings
    var gridPath = '/lib/codegrid/json/tiles/',
...
// Class for a single utf-grid
    Grid = function (tx, ty, zoom, json) {
     ...
     //change the variables latitude and longitude into array
        grid.getCode = function (lat_lng, callback) {
            console.log(lat_lng.length);
            var code_all = [];
            for (var i = 0; i < lat_lng.length; i++) {
                var x = long2tile(lat_lng[i][1], elezoom) - elex;
                var y = lat2tile(lat_lng[i][0], elezoom) - eley;
                // check error in parameters
                if ((!isInt(x)) || (!isInt(y)) || (x < 0) || (y < 0) ||
                    (x >= size) || (y >= size)) {
                    console.warn('Error in arguments to retrieve grid');
